...
def items_list_bar_view(request):
    ...
    bar=[]
    for item in Item.objects.filter(Q(state="ordenado") | Q(state="preparando"),product__place="bar", is_active=True).order_by('type','id'):
       bar.append({
           'bar':item,
...
def items_list_kitchen_view(request):
    ...
    kitchen=[]
    for item in Item.objects.filter(Q(state="ordenado") | Q(state="preparando"),product__place="cocina", is_active=True).order_by('type','id'):
       kitchen.append({
           'kitchen':item,
           'add': AddItem.objects.filter(
                            item = item
