...
def get_sales_data(request):
    if request.method == 'GET':
        ...
        if year:
            # Filtrar órdenes por año y mes, y sumar las ventas totales
            sales_data = Order.objects.filter(created_date__year=year).values('created_date__month').annotate(total_sales=Sum('total_paid')).order_by('created_date__month')
            # Convertir los datos a JSON y devolverlos
            return JsonResponse({'options': ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'], 'data': list(sales_data)})
        else:
            return JsonResponse({'error': 'Año no especificado'}, status=400)
    else:
