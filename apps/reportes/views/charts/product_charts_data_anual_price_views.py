...
@staff_member_required
def get_product_sales_price_chart(request, year,product,product2,product3):
    ...
    for group in grouped_items:
        product1_dict[months[group["month"]-1]] = round(group["total_price"],2)
        
    for group in grouped_items2:
        product2_dict[months[group["month"]-1]] = round(group["total_price"],2)
        
    for group in grouped_items3:
        product3_dict[months[group["month"]-1]] = round(group["total_price"],2)
...
@staff_member_required
def spend_product_per_customer_price_chart(request, year,product,product2,product3):
    # Filtrar los pedidos por año
    items1 = Item.objects.filter(order__shift__in_date__year=year,order__is_paid='pagada',product__name=product)
    grouped_items = items1.annotate(month=ExtractMonth("order__shift__in_date"))\
        .values("month").annotate(total_price=Sum("total_price"),cant=Sum('cant')).values("month", "total_price","cant").order_by("month")
...
@staff_member_required
def payment_product_success_price_chart(request, year,product,product2,product3):
    items1 = Item.objects.filter(order__shift__in_date__year=year,order__is_paid='pagada',product__name=product)
